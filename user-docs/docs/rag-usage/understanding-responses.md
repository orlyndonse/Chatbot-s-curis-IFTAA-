...
L'Assistant RAG Fiqh est conçu pour utiliser les documents qui vous ont été assignés par l'administrateur comme principale source d'information. Cependant, il peut y avoir des nuances dans ses réponses :

* **Réponses Basées sur Vos Documents Assignés :** Idéalement, la majorité des réponses à vos questions spécifiques sur le Fiqh Maliki proviendront des informations contenues dans les fichiers qui vous ont été assignés et que vous avez activés. L'IA essaiera d'extraire et de synthétiser l'information pertinente de ces textes.

* **Réponses Basées sur les Connaissances Générales de l'IA :**
    * Si l'IA ne trouve pas d'information pertinente dans les documents assignés et activés pour répondre à votre question, ou si la question est de nature plus générale, elle pourrait utiliser ses connaissances préexistantes.
    * **Indicateur Important :** Dans de tels cas, le système est conçu pour (ou devrait idéalement) vous informer que la réponse ne provient pas directement de vos documents assignés. Vous pourriez voir un message tel que :
    ...
    * Si vous voyez ce message, considérez la réponse comme une information générale de l'IA, qui peut être utile mais n'est pas directement tirée des sources qui vous ont été spécifiquement assignées.
...
* **Documents Assignés et Activés (Point 5.3.3.1) :**
    * La liste des "**Documents chargés**" affichée dans le [Panneau de Gestion des Documents](../interface-overview/document-management-pane.md) vous indique quels fichiers assignés sont actuellement activés et considérés comme faisant partie du contexte pour vos conversations.
...
* **Mentions de Sources dans la Réponse (Point 5.3.3.2) :**
    ...
    * Si de telles mentions directes ne sont pas présentes dans la réponse de l'IA, référez-vous à la liste des documents assignés et activés pour savoir quelles étaient les sources disponibles pour l'IA.
...
* **Pertinence :** L'IA s'efforce de fournir des réponses pertinentes. Si une réponse semble hors sujet, essayez de [reformuler votre question](./formulating-questions.md) ou de vérifier que les documents appropriés sont bien assignés et activés.
* **Exactitude :** Bien que l'IA utilise vos documents assignés, il est important de se rappeler que "Ce système RAG peut afficher des informations inexactes". Pour des sujets aussi importants que le Fiqh, il est toujours recommandé de **vérifier les informations critiques** en consultant les textes originaux ou des savants qualifiés.
