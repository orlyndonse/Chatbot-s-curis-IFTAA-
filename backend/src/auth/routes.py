# my chat application/backend/src/auth/routes.py
# Version corrigée pour Option 1 (Backend moderne + Contenu Email)
...
# from jose import ExpiredSignatureError, JWTError # Non utilisé directement ici
from pydantic import BaseModel # Gardez cet import si besoin ailleurs
...
async def create_user_account(
    ...
    try:
        ...
        # ---> CORRECTION : Lien pointant vers le FRONTEND <---
        verification_link = f"{Config.FRONTEND_URL}/verify-email?token={token}" #

        # ---> CORRECTION : Contenu HTML intégré <---
        html_content = f"""
        ...
        # Le frontend redirige vers /send-email après l'inscription réussie
...
# L'endpoint GET /verify/{token} n'est plus nécessaire pour la vérification principale,
# mais on peut le garder pour rediriger les anciens liens ou par sécurité.
...
async def login_users(
    ...
    # --- Vérification du statut 'is_verified' ---
...
async def refresh_access_token(
    ...
    # --- Vérification si l'utilisateur est toujours vérifié ---
    if not user.is_verified:
...
# RoleChecker gère la vérification is_verified
...
# L'endpoint GET /password-reset-confirm/{token} peut être gardé pour rediriger anciens liens
